...
import { SubdomainImportPanel } from '../results/SubdomainImportPanel'
...
export function ProjectWorkspace() {
  ...
  const [methodologyPanelTab, setMethodologyPanelTab] = useState<'guidance' | 'notes'>('guidance')
  ...
  useEffect(() => {
    if (selectedVulnerabilityId || tab !== 'flow') return
    const first = methodology.data?.find((category) => category.vulnerabilities.length)?.vulnerabilities[0]
    if (first) setSelectedVulnerabilityId(first.id)
  }, [methodology.data, selectedVulnerabilityId, tab])
  useEffect(() => { setMethodologyPanelTab('guidance') }, [selectedVulnerabilityId])
  ...
  return <main className="page target-workspace">
    ...
    {tab === 'target' && <section className="target-tab">
      ...
      <SubdomainImportPanel projectId={projectId} compact />
    ...
    {tab === 'flow' && <section className="target-flow-tab methodology-tab"><div className="methodology-workbench"><aside className="panel methodology-checklist">{methodology.data?.map((category) => { const tested = category.vulnerabilities.filter((item) => item.status === 'tested_clean' || item.status === 'vulnerable').length; return <details key={category.id} open><summary><span style={{ color: category.accent }}>{category.icon}</span><b>{category.name}</b><small>{tested}/{category.vulnerabilities.length} tested</small></summary><div>{category.vulnerabilities.map((item) => <button className={item.id === selectedVulnerabilityId ? 'selected' : ''} onClick={() => setSelectedVulnerabilityId(item.id)} key={item.id}><span>{methodologyStatusLabels[item.status]}</span><b>{item.name}</b></button>)}</div></details> })}{methodology.isPending && <div className="empty compact">Loading testing checklist…</div>}{methodology.isError && <p className="error">The testing methodology could not be loaded.</p>}</aside><article className="panel methodology-detail">{!selectedVulnerabilityId ? <div className="methodology-placeholder"><strong>Select a check</strong><span>Choose a vulnerability from the checklist to review its guidance and record coverage.</span></div> : methodologyDetail.isPending ? <div className="empty">Loading check…</div> : methodologyDetail.isError ? <p className="error">This methodology check could not be loaded.</p> : methodologyDetail.data && <><header><div><p className="eyebrow">{methodologyDetail.data.category_name}</p><h2>{methodologyDetail.data.name}</h2></div><select value={methodologyDetail.data.status} onChange={(event) => updateMethodology.mutate({ status: event.target.value as MethodologyStatus, notes: methodologyDetail.data?.notes ?? '' })}>{methodologyStatuses.map((status) => <option value={status} key={status}>{methodologyStatusLabels[status]}</option>)}</select></header><nav className="methodology-detail-tabs" aria-label="Testing check detail"><button className={methodologyPanelTab === 'guidance' ? 'selected' : ''} onClick={() => setMethodologyPanelTab('guidance')}>Guidance</button><button className={methodologyPanelTab === 'notes' ? 'selected' : ''} onClick={() => setMethodologyPanelTab('notes')}>Notes</button></nav>{methodologyPanelTab === 'guidance' ? <div className="methodology-description"><MarkdownContent source={methodologyDetail.data.description || 'No testing guidance is available for this check yet.'} /></div> : <label className="methodology-notes-panel">Check notes<textarea value={methodologyDetail.data.notes} onChange={(event) => queryClient.setQueryData(['projects', projectId, 'methodology', selectedVulnerabilityId], { ...methodologyDetail.data, notes: event.target.value })} placeholder="Record evidence, endpoints, payloads, and conclusions…" /></label>}{methodologyPanelTab === 'notes' && <footer><button disabled={updateMethodology.isPending} onClick={() => updateMethodology.mutate({ status: methodologyDetail.data!.status, notes: methodologyDetail.data!.notes })}>{updateMethodology.isPending ? 'Saving…' : 'Save check'}</button>{updateMethodology.isError && <p className="error">{updateMethodology.error.message}</p>}</footer>}</>}</article></div></section>}
