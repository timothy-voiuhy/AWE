...
const palette: Record<string, string> = {
  ...
  platform: '#89dceb', component: '#94e2d5', misconfiguration: '#fab387', cloud_asset: '#cba6f7',
  cloudflare_zone: '#89b4fa', dns_record: '#74c7ec', cluster: '#a6e3a1', workload: '#a6e3a1',
  container_image: '#f9e2af', identity_provider: '#f5c2e7', oidc_endpoint: '#a6e3a1', secret: '#f38ba8',
}
...
export function NetworkGraphPage() {
  ...
  const selectedEvidence = useQuery({ queryKey: ['projects', projectId, 'evidence', 'entity', selected?.id], queryFn: () => api.listEvidence(projectId, { entity_id: selected!.id }), enabled: Boolean(selected?.id), refetchInterval: 5000 })
  useEffect(() => {
    setAliasValues(selected?.aliases || [])
    setAliasDraft('')
    setConfidenceDraft(String(selected?.confidence ?? 1))
    setEvidenceNote('')
  }, [selected?.id])
  const identityCandidates = useMemo(() => {
    if (!graphView) return []
    const groups = new Map<string, GraphEntity[]>()
    graphView.entities.forEach(entity => {
      const normalized = (entity.value || entity.label).toLowerCase().trim().replace(/^https?:\/\//, '').replace(/\/$/, '')
      if (!normalized) return
      const key = `${entity.kind}:${normalized}`
      groups.set(key, [...(groups.get(key) || []), entity])
    })
    return [...groups.values()].filter(group => group.length > 1).map(group => ({ key: `${group[0].kind}:${group[0].value || group[0].label}`, entities: group }))
  }, [graphView?.entities])
  const selectedIdentityPeers = useMemo(() => identityCandidates.find(group => group.entities.some(entity => entity.id === selected?.id))?.entities.filter(entity => entity.id !== selected?.id) || [], [identityCandidates, selected?.id])
  ...
  const updateIdentity = useMutation({ mutationFn: () => { if (!selected) throw new Error('Select an entity first'); return api.updateGraphIdentity(projectId, investigationId, selected.id, { canonical_id: selected.canonical_id || selected.id, aliases: aliasValues, confidence: Math.max(0, Math.min(1, Number(confidenceDraft) || 0)) }) }, onSuccess: () => { setNotice('Identity metadata saved.'); void queryClient.invalidateQueries({ queryKey: ['projects', projectId, 'investigation-graph', investigationId] }) } })
  const mergeEntity = useMutation({ mutationFn: (sourceId: string) => { if (!selected) throw new Error('Select a canonical entity first'); return api.mergeGraphEntity(projectId, investigationId, sourceId, selected.id) }, onSuccess: result => { setSelectedId(result.entity.id); selectedIdRef.current = result.entity.id; setNotice(`Merged ${result.merged_entity_ids.length} duplicate entit${result.merged_entity_ids.length === 1 ? 'y' : 'ies'}; rewired ${result.rewired_relationships} relationship${result.rewired_relationships === 1 ? '' : 's'}.`); void queryClient.invalidateQueries({ queryKey: ['projects', projectId, 'investigation-graph', investigationId] }) } })
  const transform = useMutation({ mutationFn: () => { if (activeTransform?.requires_approval && !window.confirm(`This is an ${activeTransform.mode} transform and may actively interact with the target. Continue?`)) throw new Error('Active transform cancelled by user.'); return api.startGraphTransform(projectId, { transform_id: selectedTransform, entity_ids: selected ? [selected.id] : [], parameters: transformParameters, investigation_id: investigationId, approved: activeTransform?.requires_approval === true }) }, onSuccess: job => { setTransformJobId(job.id); setNotice(`Transform queued: ${activeTransform?.display_name || job.transform_id}.`); setSelectedTransform(''); setTransformParameters({}) } })
  ...
  const retryTransform = useMutation({ mutationFn: () => { const job = transformJob.data; if (!job) throw new Error('No transform job is available'); return api.startGraphTransform(projectId, { transform_id: job.transform_id, entity_ids: job.entity_ids, parameters: job.parameters, investigation_id: job.investigation_id, approved: true, source_job_id: job.id }) }, onSuccess: job => { setTransformJobId(job.id); setNotice(`Retry queued for ${job.transform_id}.`) } })
  const createEntityEvidence = useMutation({
    mutationFn: () => {
      if (!selected) throw new Error('Select an entity first')
      return api.createEvidence(projectId, {
        title: `Note for ${selected.label}`,
        summary: evidenceNote,
        kind: 'note',
        source_type: 'network_graph',
        source_id: selected.id,
        investigation_id: investigationId,
        entity_ids: [selected.id],
        relationship_ids: [],
        tags: [selected.kind],
        data: { entity_label: selected.label, entity_value: selected.value || selected.label },
      })
    },
    onSuccess: () => {
      setEvidenceNote('')
      setNotice('Evidence note saved.')
  ...
  const addAlias = () => {
    ...
    if (alias && !aliasValues.includes(alias) && alias !== selected?.label && alias !== selected?.value) setAliasValues(current => [...current, alias].slice(0, 100))
    setAliasDraft('')
  }
  useEffect(() => {
  ...
  const minimapClick = (event: ReactMouseEvent<HTMLCanvasElement>) => {
    const cy = cyRef.current
    const canvas = minimapRef.current
    if (!cy || !canvas || !cy.nodes().length) return
    const rect = canvas.getBoundingClientRect()
    const bounds = cy.nodes().boundingBox()
    const scale = Math.min((rect.width - 12) / Math.max(bounds.w, 1), (rect.height - 12) / Math.max(bounds.h, 1))
    const point = { x: bounds.x1 + (event.clientX - rect.left - 6) / scale, y: bounds.y1 + (event.clientY - rect.top - 6) / scale }
    let nearest = cy.nodes()[0]
  ...
  const graphControls = <section className="graph-control-panel">
    ...
    {displayMenuOpen && <div className="graph-display-popover"><header><b>Display &amp; relationships</b><button onClick={() => setDisplayMenuOpen(false)} aria-label="Close display controls">×</button></header><label className="graph-display-row">Layout<select value={layoutMode} onChange={event => setLayoutMode(event.target.value as LayoutMode)}><option value="force">Force-directed</option><option value="hierarchy">Hierarchy</option><option value="radial">Radial target map</option><option value="concentric">Concentric</option><option value="timeline">Timeline</option></select></label><label className="graph-display-row">Edge shape<select value={edgeCurve} onChange={event => setEdgeCurve(event.target.value as EdgeCurve)}><option value="bezier">Curved</option><option value="straight">Straight</option><option value="taxi">Orthogonal</option></select></label><label className="graph-display-check"><input type="checkbox" checked={showArrows} onChange={event => setShowArrows(event.target.checked)}/> Arrowheads</label><label className="graph-display-check"><input type="checkbox" checked={showEdgeLabels} onChange={event => setShowEdgeLabels(event.target.checked)}/> Relationship labels</label><div className="graph-display-kinds"><small>Relationship types</small>{relationshipKinds.length ? relationshipKinds.map(item => <label key={item}><input type="checkbox" checked={!visibleRelationshipKinds || visibleRelationshipKinds.includes(item)} onChange={() => setVisibleRelationshipKinds(current => { if (!current) return relationshipKinds.filter(kind => kind !== item); return current.includes(item) ? current.filter(kind => kind !== item) : [...current, item] })}/>{item}</label>) : <span>No relationships</span>}<button onClick={() => setVisibleRelationshipKinds(null)} disabled={!visibleRelationshipKinds}>Show all types</button></div></div>}
    {selectionMenuOpen && <div className="graph-selection-popover"><header><b>Selection tools</b><button onClick={() => setSelectionMenuOpen(false)} aria-label="Close selection tools">×</button></header><div className="graph-selection-actions"><button onClick={() => setLassoMode(value => !value)}>{lassoMode ? 'Disable area select' : 'Enable area select'}</button><button onClick={copySelection} disabled={!selectedEntities.length}>Copy values</button><button onClick={exportSelection} disabled={!selectedEntities.length}>Export selection</button><button onClick={showSelectedNeighborhood} disabled={!selectedIds.length}>{neighborhoodOnly ? 'Show full graph' : 'Selected neighborhood'}</button><button onClick={showSelectedComponents} disabled={!selectedIds.length}>{componentOnly ? 'Show full graph' : 'Connected component'}</button></div><label className="graph-selection-depth">Neighborhood depth<select value={analysisDepth} onChange={event => setAnalysisDepth(Number(event.target.value))}><option value={1}>1 hop</option><option value={2}>2 hops</option><option value={3}>3 hops</option></select></label><form className="graph-selection-save" onSubmit={event => { event.preventDefault(); saveSelectionSet() }}><input value={selectionSetName} onChange={event => setSelectionSetName(event.target.value)} placeholder="Selection set name"/><button disabled={!selectedIds.length || !selectionSetName.trim()}>Save set</button></form>{selectionSets.length > 0 && <div className="graph-selection-sets"><small>Saved sets</small>{selectionSets.map(item => <div key={item.name}><button onClick={() => applySelectionSet(item)} title={`Load ${item.name}`}>{item.name}<span>{item.ids.length}</span></button><button onClick={() => deleteSelectionSet(item.name)} aria-label={`Delete ${item.name}`}>×</button></div>)}</div>}</div>}
  </section>

  return <main className={`page feature-page network-page${graphExpanded ? ' network-expanded' : ''}`}>
    <section className="graph-workbench"><section className="panel graph-canvas"><div ref={hostRef} className="cytoscape-host" /><canvas ref={minimapRef} className="graph-minimap" aria-label="Graph minimap" onClick={minimapClick} />{graphExpanded && <button className="graph-expanded-exit" onClick={() => setGraphExpanded(false)} aria-label="Exit full workspace">×</button>}{touchLabel && <div className="graph-touch-label" style={{ left: touchLabel.x, top: touchLabel.y }} aria-live="polite">{touchLabel.text}</div>}{mobileTouchNodeId && selected && <div className="graph-touch-actions" aria-label="Selected node actions"><button onClick={() => setMobileInspectorOpen(true)}>Inspect</button><button onClick={() => { setFocusId(mobileTouchNodeId); setMobileMenuId('') }}>Focus</button><button onClick={() => void navigator.clipboard?.writeText(selected.value || selected.label)}>Copy</button></div>}{mobileMenuId && selected && <div className="graph-touch-menu" aria-label="Node actions"><button onClick={() => { setMobileInspectorOpen(true); setMobileMenuId('') }}>◉<span>Inspect</span></button><button onClick={() => { setFocusId(mobileMenuId); setMobileMenuId('') }}>◎<span>Focus</span></button><button onClick={() => { void navigator.clipboard?.writeText(selected.value || selected.label); setMobileMenuId('') }}>⧉<span>Copy</span></button><button onClick={() => setMobileMenuId('')}>×<span>Close</span></button></div>}{graph.isPending && !graph.data && <div className="graph-overlay">Loading graph…</div>}{!visibleEntities.length && !graph.isPending && <div className="graph-overlay">No entities match the active filters.</div>}</section>
      <aside className={`panel graph-inspector${mobileInspectorOpen ? ' mobile-inspector-open' : ''}`}>
        {graphControls}
        {graphStatus}
        {selected ? <>
          <section className="graph-selected-card">
            <header><div><b>{selected.label}</b><span>{selected.kind} · {selected.source}</span></div><button className="graph-mobile-close" onClick={() => setMobileInspectorOpen(false)} aria-label="Close inspector">×</button><button className="danger" onClick={() => { if (selected.source === 'manual' && confirm('Delete this analyst entity?')) deleteEntity.mutate(selected.id) }}>Delete</button></header>
            <dl className="graph-properties"><dt>Value</dt><dd><code>{selected.value || selected.label}</code></dd><dt>Confidence</dt><dd>{Math.round(selected.confidence * 100)}%</dd><dt>Relationships</dt><dd>{selectedRelationships.length}</dd></dl>
            <div className="graph-actions"><select value={selectedTransform} onChange={event => { setSelectedTransform(event.target.value); setTransformParameters({}) }}><option value="">Run transform…</option>{transforms.data?.filter(item => item.input_types.includes(selected.kind)).map(item => <option value={item.id} key={item.id}>{item.display_name}{item.requires_approval ? ' · approval' : ''}</option>)}</select><button disabled={!selectedTransform || transform.isPending} onClick={() => transform.mutate()}>Run</button></div>
            {activeTransform && <div className="transform-parameters"><small>{activeTransform.description || 'Transform parameters'}</small>{activeTransform.parameters.map(parameter => { const key = String(parameter.key || ''); return <label key={key}>{String(parameter.label || key)}<input value={transformParameters[key] || String(parameter.default || '')} onChange={event => setTransformParameters({ ...transformParameters, [key]: event.target.value })} placeholder={key} /></label> })}<span className={`transform-risk risk-${activeTransform.mode}`}>{activeTransform.mode}{activeTransform.requires_approval ? ' · approval required' : ''}</span></div>}
            <div className="graph-relationships"><b>Relationships</b>{selectedRelationships.map(edge => <span key={edge.id}>{edge.kind}: {edge.source_id === selected.id ? edge.target_id : edge.source_id}</span>)}</div>
            <div className="graph-provenance"><b>Provenance</b>{selected.provenance.length ? selected.provenance.map((item, index) => <span key={`${String(item.source || item.tool_key || 'source')}-${index}`}>{String(item.source || item.tool_key || 'source')}{item.tool_key ? ` · ${String(item.tool_key)}` : ''}{item.transform_job_id ? ` · job ${String(item.transform_job_id)}` : ''}</span>) : <small>No source metadata recorded.</small>}</div>
            <section className="graph-evidence-panel"><header><b>Evidence</b><span>{selectedEvidence.data?.length || 0} linked</span></header>{selectedEvidence.data?.slice(0, 4).map(item => <article key={item.id}><strong>{item.title}</strong><small>{item.kind} · {new Date(item.created_at).toLocaleString()}</small><p>{item.summary || item.source_type}</p></article>)}{!selectedEvidence.isPending && !selectedEvidence.data?.length && <small>No linked evidence yet.</small>}<textarea value={evidenceNote} onChange={event => setEvidenceNote(event.target.value)} placeholder="Add evidence note for this entity..." /><button onClick={() => createEntityEvidence.mutate()} disabled={!evidenceNote.trim() || createEntityEvidence.isPending}>{createEntityEvidence.isPending ? 'Saving...' : 'Save evidence note'}</button>{createEntityEvidence.isError && <p className="error">{createEntityEvidence.error.message}</p>}</section>
            <details><summary>Properties</summary><pre>{JSON.stringify(selected.data, null, 2)}</pre></details>
          </section>
          <section className="graph-identity-workbench"><header><div><b>Identity review</b><span>{selected.canonical_id ? `Canonical: ${selected.canonical_id}` : 'No canonical identity assigned'}</span></div><small>{identityCandidates.length} duplicate group{identityCandidates.length === 1 ? '' : 's'} detected</small></header>{selectedIdentityPeers.length ? <div className="graph-identity-candidates"><b>Potential duplicates for this entity</b>{selectedIdentityPeers.map(peer => <div key={peer.id}><span><strong>{peer.label}</strong><small>{peer.kind} · {peer.source} · {Math.round(peer.confidence * 100)}%</small></span><button disabled={selected.source === 'derived' || peer.source === 'derived' || mergeEntity.isPending} title={selected.source === 'derived' || peer.source === 'derived' ? 'Derived entities must first be materialized as analyst entities' : 'Merge this persisted entity into the selected canonical entity'} onClick={() => { if (confirm(`Merge ${peer.label} into ${selected.label}?`)) mergeEntity.mutate(peer.id) }}>Merge into selected</button></div>)}</div> : <p className="muted">No exact kind/value duplicate was detected for the selected entity.</p>}<form className="graph-identity-form" onSubmit={event => { event.preventDefault(); updateIdentity.mutate() }}><label>Confidence<input type="number" min="0" max="1" step="0.05" value={confidenceDraft} onChange={event => setConfidenceDraft(event.target.value)} /></label><label>Alias<input value={aliasDraft} onChange={event => setAliasDraft(event.target.value)} placeholder="Alternate label or value" onKeyDown={event => { if (event.key === 'Enter') { event.preventDefault(); addAlias() } }} /></label><button type="button" onClick={addAlias} disabled={!aliasDraft.trim()}>Add alias</button><button type="submit" disabled={selected.source === 'derived' || updateIdentity.isPending}>{updateIdentity.isPending ? 'Saving…' : 'Save identity'}</button></form>{aliasValues.length ? <div className="graph-alias-list"><small>Aliases</small>{aliasValues.map(alias => <button type="button" key={alias} onClick={() => setAliasValues(current => current.filter(item => item !== alias))} title="Remove alias">{alias} ×</button>)}</div> : null}</section>
        </> : <div className="empty">Select an entity to inspect it.</div>}
        {transformJob.data && <section className={`graph-transform-activity transform-${transformJob.data.status}`}><header><div><b>Transform activity</b><span>{transformJob.data.transform_id} · {transformJob.data.status}</span></div>{['queued', 'running'].includes(transformJob.data.status) ? <button onClick={() => cancelTransform.mutate()} disabled={cancelTransform.isPending}>Cancel</button> : ['failed', 'cancelled'].includes(transformJob.data.status) ? <button onClick={() => retryTransform.mutate()} disabled={retryTransform.isPending}>Retry</button> : <span className="transform-complete">{transformJob.data.outputs_ingested ? 'Outputs ingested' : 'Completed'}</span>}</header><div className="transform-progress-track"><span style={{ width: `${transformJob.data.progress_total ? Math.min(100, Math.round(((transformJob.data.progress_completed || 0) / transformJob.data.progress_total) * 100)) : ['completed', 'failed', 'cancelled'].includes(transformJob.data.status) ? 100 : 18}%` }} /></div><div className="transform-activity-meta"><span>{transformJob.data.progress_total ? `${transformJob.data.progress_completed || 0}/${transformJob.data.progress_total} steps` : transformJob.data.message || 'Waiting for Docker operation…'}</span><button onClick={() => setTransformJobId('')} aria-label="Dismiss transform activity">×</button></div>{transformJob.data.logs?.length ? <details><summary>Recent logs ({transformJob.data.logs.length})</summary><pre>{transformJob.data.logs.slice(-8).join('\n')}</pre></details> : null}</section>}
