...
function PipelineParameterEditor({ steps, tools, values, onChange }: { steps: PipelineTemplate['steps']; tools: Awaited<ReturnType<typeof api.listDockerTools>> | undefined; values: Record<string, unknown>; onChange: (key: string, value: unknown) => void }) {
  const specs = steps.flatMap(step => (tools?.find(tool => tool.key === step.tool_key)?.param_specs || []).map(spec => ({ ...spec, tool_key: step.tool_key })))
  const seen = new Set<string>()
  return <section className="pipeline-parameter-editor"><header><b>Tool parameters</b><small>Credentials are sent only for this run and redacted from saved session metadata.</small></header>{specs.filter(spec => { if (seen.has(spec.key)) return false; seen.add(spec.key); return true }).map(spec => <label key={spec.key}>{spec.label || `${spec.tool_key} · ${spec.key}`}{spec.type === 'check' ? <input type="checkbox" checked={Boolean(values[spec.key] ?? spec.default ?? false)} onChange={event => onChange(spec.key, event.target.checked)} /> : spec.type === 'combo' ? <select value={String(values[spec.key] ?? spec.default ?? '')} onChange={event => onChange(spec.key, event.target.value)}>{(spec.options || []).map(option => <option key={option}>{option}</option>)}</select> : <input type={spec.type === 'secret' || spec.default === 'secret' ? 'password' : 'text'} value={String(values[spec.key] ?? (spec.default === 'secret' ? '' : spec.default ?? ''))} onChange={event => onChange(spec.key, event.target.value)} />}</label>)}</section>
}

export function PipelinePage() {
  const { projectId = '' } = useParams(); const qc = useQueryClient()
  const [pipelineKey,setPipelineKey]=useState(''); const [sessionId,setSessionId]=useState(''); const [jobId,setJobId]=useState(''); const [tab,setTab]=useState<Tab>('configuration'); const [checked,setChecked]=useState<Set<string>>(new Set()); const [toolLog,setToolLog]=useState(''); const [pipelineParams,setPipelineParams]=useState<Record<string,unknown>>({})
  ...
  const dockerTools=useQuery({queryKey:['docker','tools'],queryFn:api.listDockerTools})
  ...
  const start=useMutation({mutationFn:(input:{pipeline:string;session?:string;tools?:string[];approved?:boolean})=>api.startPipelineRun(projectId,input.pipeline,{session_id:input.session,tool_keys:input.tools,params:pipelineParams,approved:input.approved}),onSuccess:job=>{setJobId(job.id);setTab('monitor');setChecked(new Set());void qc.invalidateQueries({queryKey:['projects',projectId,'pipeline-runs']})}})
  ...
  function activeKeys(keys:string[]){return keys.filter(key=>['active','high_risk'].includes(dockerTools.data?.find(tool=>tool.key===key)?.execution_mode||''))}
  function runTools(keys:string[]){if(!template||!sessionId||!keys.length)return;const active=activeKeys(keys);if(active.length&&!window.confirm(`Run active tools: ${active.join(', ')}? These may interact with the target or cloud account.`))return;start.mutate({pipeline:template.key,session:sessionId,tools:keys,approved:active.length>0})}
  function runPipeline(key:string){const active=activeKeys(pipelines.data?.find(item=>item.key===key)?.steps.map(step=>step.tool_key)||[]);if(active.length&&!window.confirm(`Run active tools: ${active.join(', ')}? These may interact with the target or cloud account.`))return;start.mutate({pipeline:key,approved:active.length>0})}
  ...
  {tab==='configuration'&&<div className="pipeline-config"><label>Pipeline<select value={pipelineKey} onChange={e=>selectPipeline(e.target.value)}>{pipelines.data?.map(p=><option value={p.key} key={p.key}>{p.name}</option>)}</select></label><label>Target<input value={project.data?.target??''} readOnly/></label>{template&&<PipelineParameterEditor steps={template.steps} tools={dockerTools.data} values={pipelineParams} onChange={(key,value)=>setPipelineParams(current=>({...current,[key]:value}))}/>} {template&&<><p>{template.description}</p><div className="pipeline-step-preview">{stages.map(([stage,steps])=><section key={stage}><b>Stage {stage}</b>{steps.map(step=><div key={step.tool_key}><code>{step.tool_key}</code><span>{step.condition}{step.input_category?` · input: ${step.input_category}`:''}</span></div>)}</section>)}</div></>}</div>}
