...
class EventHook:
    """Minimal thread-safe signal used by non-Qt application services."""
    ...
    def emit(self, *args) -> None:
        ...
        for callback in callbacks:
            try:
                ...
                logger.exception("Pipeline event callback failed")


class PipelineRunner:
    """Framework-neutral pipeline execution service.

    It may run in a QThread, a backend-owned thread, or synchronously in tests.
    Consumers subscribe to EventHook attributes before calling run().
    """
    ...
    def __init__(
        ...
        self.step_log = EventHook()
        self.step_done = EventHook()
        self.stage_done = EventHook()
        self.pipeline_done = EventHook()
