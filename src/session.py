...
class SessionHandler:
    ...
    def createServer(self):
        ...
        while retry_count < max_retries:
            try:
                ...
                if e.errno in (errno_mod.EADDRINUSE, 10048):  # Port already in use (Linux/Windows)
                    retry_count += 1
                    old_port = self.server_port
                    self.server_port = random.randint(8182, 9999)
                    logging.warning(f"Port {old_port} is already in use. Trying port {self.server_port} instead.")
                else:
                    raise  # Re-raise other socket errors
        
        if retry_count >= max_retries:
            logging.error(f"Failed to bind to a port after {max_retries} attempts. Exiting.")
            return
            
        self.server_socket.listen()
        logging.info(f"Session handler listening on port {self.server_port}")
        
        # Create a file to store the current port for clients to read
        port_file_path = os.path.join(self.runDir, "tmp", "session_port.txt")
        os.makedirs(os.path.dirname(port_file_path), exist_ok=True)
        with open(port_file_path, "w") as port_file:
            port_file.write(str(self.server_port))
    ...
    def createClientConnection(self):
        # Read the port from the file if possible
        port_file_path = os.path.join(self.runDir, "tmp", "session_port.txt")
        try:
            ...
            # If file doesn't exist or contains invalid data, use the default port
            pass
