...
def _json_to_html(raw: bytes) -> bytes:
    ...
    # Tokenise the pretty-printed text (which contains only plain ASCII-safe
    # JSON tokens) then HTML-escape each token individually so we never
    # double-escape or corrupt span tags.
    _TOKEN = re.compile(
        r'(?P<key>"(?:[^"\\]|\\.)*"(?=\s*:))'   # object key  (before colon)
        r'|(?P<str>"(?:[^"\\]|\\.)*")'           # string value
        r'|(?P<lit>true|false|null)'             # literals
        r'|(?P<num>-?\d+(?:\.\d+)?(?:[eE][+-]?\d+)?)'  # number
    )
    _CSS = {"key": "jk", "str": "js", "lit": "jb", "num": "ji"}
    # null gets its own colour; true/false share jb
    def _repl(m: re.Match) -> str:
        kind = m.lastgroup
        tok  = _html.escape(m.group(0))
        if kind == "lit" and m.group(0) == "null":
            return f'<span class="jn">{tok}</span>'
        return f'<span class="{_CSS[kind]}">{tok}</span>'

    # Characters outside tokens (brackets, colons, commas, whitespace) are
    # passed through after HTML-escaping via re.sub with a fallback pattern.
...
# A named QWebEngineProfile spawns its own QtWebEngineProcess; creating one
# per widget instance would give N separate ~300 MB processes.  A singleton
# keeps it to one extra process total, and that process is killed by Chromium
# when all pages referencing it are deleted.
...
class ResponseRenderView(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self._web:  object | None = None  # QWebEngineView  — created on demand
        self._page: object | None = None  # QWebEnginePage  — created on demand

        # Idle timer — no WebEngine imports needed here, just Qt core
    ...
    def render_response(self, body: bytes, content_type: str = "", base_url: str = "") -> None:
        try:
            _get_render_profile()   # ensures the module import succeeds
        except Exception:
            return
