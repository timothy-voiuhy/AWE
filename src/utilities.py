...
def runWhoisOnTarget(server_name, project_dir_path=None):
    # Require at least one dot — bare names like "testprj" are not valid targets
    if not server_name or "." not in server_name:
        logging.warning(f"whois skipped: '{server_name}' is not a valid domain or IP")
        return b""
